...
test('calibrated boost raises quiet speech by 12 dB without changing silence', () => {
...
test('unity and invalid gain values behave predictably', () => {
    ...
    for(const value of [NaN,Infinity,-1,0,100]) assert.equal(peak(new QuartzOutputLevel(value).process(tone(1000))),4000);
});
